...
describe('POST /v1/embeddings', () => {
  ...
  it('returns 400 for missing input', async () => {
    const { status, body } = await request(app, 'POST', '/v1/embeddings', { model: 'text-embedding-004' });
    expect(status).toBe(400);
    expect(body.error.type).toBe('invalid_request_error');
  });

  it('returns 400 for empty string input', async () => {
    const { status, body } = await request(app, 'POST', '/v1/embeddings', { input: '' });
    expect(status).toBe(400);
    expect(body.error.type).toBe('invalid_request_error');
  });
  ...
  it('uses batch endpoint for array input', async () => {
    ...
    vi.spyOn(global, 'fetch').mockImplementation(async (url, init) => {
      ...
      if (capturedUrl.includes('batchEmbedContents')) {
  ...
  it('auto-routing: uses google when no model specified', async () => {
    ...
    const { status } = await request(app, 'POST', '/v1/embeddings', { input: 'test' });
  ...
  it('auto-routing: falls back to Mistral when Google has no key', async () => {
    ...
    const { status, body } = await request(app, 'POST', '/v1/embeddings', { input: 'test' });
